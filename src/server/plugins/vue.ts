...
export const vuePlugin: Plugin = ({ root, app }) => {
    ...
    function compileSFCMain(descriptor: SFCDescriptor, pathname: string, timestamp: string | undefined): string {
    ...
    function compileSFCTemplate(
        ...
        const { code, errors } = resolveCompiler(root).compileStyle({
            ...
            filename,
            compilerOptions: {
                scopedId: scoped ? `data-v-${hash_sum(pathname)}` : null,
